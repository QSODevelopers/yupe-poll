...
/**
 * This is the model class for table "{{poll_poll}}".
 *
 * The followings are the available columns in table '{{poll_poll}}':
 * @property string $id
 ...
 */
class Poll extends yupe\models\YModel
{
	/**
	...
	{
		return array(
			'id'                => Yii::t('PollModule.poll', 'ID'),
			'title'             => Yii::t('PollModule.poll', 'Title'),
			'description'       => Yii::t('PollModule.poll', 'Description'),
			'status'            => Yii::t('PollModule.poll', 'Status'),
		);
	}
	...
	{
		return array(
			self::STATUS_CLOSED => Yii::t('PollModule.poll', 'Closed'),
			self::STATUS_OPEN   => Yii::t('PollModule.poll', 'Open'),
		);
	}
